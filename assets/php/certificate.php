...
for ($row = 1; $row <= $lastRow; $row++) {
    ...
    if ($student == 'ФИО' || $student == 'фио') {
        continue;
    }
    //для каждого ученика создаем изображение - грамоту
    ...
    // header("Content-Type: image/$filetype");
    imagejpeg($img, dirname(__DIR__) . '\Сертификаты\\' . $student . date('dmY-hi') . '_' . $hash . "." . $filetype);
    $_SESSION["message"] = "Грамоты сохранены в папку assets/Сертификаты";
}
